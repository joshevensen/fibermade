...
export const action = async ({ request }: ActionFunctionArgs) => {
  try {
    ...
    await client.updateColorway(result.identifiableId, {
      status: "retired",
    });
    ...
    await client.createIntegrationLog(connection.fibermadeIntegrationId, {
      ...
      status: "success",
      message: `Retired Colorway #${result.identifiableId} (Shopify product ${productId} deleted)`,
      metadata: { shopify_product_id: productId, shopify_gid: productGid },
      synced_at: new Date().toISOString(),
    });
  } catch (err) {
    console.error(
      "[webhooks.products.delete] Error:",
