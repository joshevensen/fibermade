...
- Status badge colors should follow the existing pattern in `OrderEditPage.vue` `getStatusBadgeClass()` -- extend it with the new statuses
...
- [ ] Status filter dropdown includes all six status options
...
- **Existing `getStatusBadgeClass()`**: The `OrderEditPage.vue` already has a `getStatusBadgeClass()` function with mappings for draft, open, closed, cancelled. This needs to be updated with the new statuses. Consider whether to extract this to a shared utility, but per Non-Goals, keep it simple -- just update both files independently.
...
- `platform/resources/js/pages/creator/orders/OrderEditPage.vue` -- `getStatusBadgeClass()` function to update
...
- Modify `platform/resources/js/pages/creator/orders/OrderIndexPage.vue` -- add status badge slot with color coding, verify sort order
- Modify `platform/resources/js/pages/creator/orders/OrderEditPage.vue` -- update `getStatusBadgeClass()` with new statuses
