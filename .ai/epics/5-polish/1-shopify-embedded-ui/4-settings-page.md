...
- Settings stored in the Integration's `settings` JSON field alongside the existing `shop` key:
  ...
  ```
- Use Polaris form components: `FormLayout`, `Checkbox` (for auto-sync toggle), `ChoiceList` (for collection selection)
...
- [ ] Route `app.settings.tsx` exists and renders at `/app/settings`
- [ ] Loader fetches integration settings and collections list
- [ ] Redirects to `/app` if shop is not connected
- [ ] Auto-sync toggle (checkbox) defaults to on
- [ ] Collection list shows all synced collections with checkboxes
- [ ] Collections are checked by default (opt-out model: uncheck to exclude)
- [ ] Save button submits settings via `updateIntegration()`
- [ ] Success banner shown after saving
- [ ] Error banner shown if save fails
- [ ] Existing `shop` setting preserved when saving (merged, not overwritten)
...
- **Loader data**: Fetch two things in parallel:
  ...
  2. Collections via `getCollections()` -- for the collection picker. Pass `limit: 100` to get all collections.
- **Collection display**: Each collection has `id`, `name`, and `colorways_count`. Show as a `ChoiceList` with `allowMultiple` where each option is a collection. Checked = synced (not excluded). This is an opt-out model.
...
- `shopify/app/services/fibermade-client.server.ts` -- `getIntegration()`, `updateIntegration()`, `getCollections()`
