...
- Route: `GET /store/orders/{order}` in `routes/store.php`
...
- [ ] Controller action:
  - Loads order with `orderItems.colorway.media`, `orderItems.base`, `orderable`
  - Verifies the order belongs to the authenticated store (403 if not)
...
- [ ] Skein count and colorway count displayed somewhere visible (header or summary)
- [ ] Tests: controller tests for authorization, data loading, correct grouping
...
- **Page consistency**: This page should visually match the `OrderReviewPage.vue` from Story 3.3 as closely as possible, just without the inputs, buttons, and minimums feedback. Consider extracting shared layout components (colorway row, base row, pricing summary) to reusable Vue components that both pages can use.
